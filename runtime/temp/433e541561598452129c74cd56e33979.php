<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:78:"C:\PHPStudyE\WWW\lyshopV2.2\public/../application/admin\view\product\edit.html";i:1510216092;s:79:"C:\PHPStudyE\WWW\lyshopV2.2\public/../application/admin\view\Public\header.html";i:1510801998;}*/ ?>
<!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" id="name" name="name" required="" lay-verify="required"
                       autocomplete="off" class="layui-input" value="<?php echo $product['name']; ?>">
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <select name="cid" lay-verify="required" id="cid">
                    <?php if(is_array($category) || $category instanceof \think\Collection || $category instanceof \think\Paginator): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                        <option value="<?php echo $vo['id']; ?>" <?php if($vo['id'] == $product['cid']): ?> checked <?php endif; ?> ><?php echo $vo['name']; ?></option>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" id="price" name="price" required="" lay-verify="required"
                       autocomplete="off" class="layui-input" value="<?php echo $product['price']; ?>">
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" id="cost_price" name="cost_price"
                       autocomplete="off" class="layui-input" value="<?php echo $product['cost_price']; ?>">
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline" >
                <input class="layui-input" name="first_img" value="<?php echo $product['first_img']; ?>" id="first_img">
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" id="keyword" name="keyword" required="" lay-verify="required"
                       autocomplete="off" class="layui-input" value="<?php echo $product['keyword']; ?>">
            </div>
        </div>
...
<script>
    layui.use(['layedit', 'upload', 'form', 'layer'], function () {
        ...
        $("#additem").click(function(){
            ...
            $.post("<?php echo url('Product/updateData'); ?>", product_data, function (res) {
                if(res == 1){
        ...
        /*form.on('submit(add)', function (data) {
            layedit.sync(editIndex);
            //发异步，把数据提交给php
            $.post("<?php echo url('Product/updateData'); ?>",{"field":data.field},function(res){
                if(res == 1){
                    layer.alert("修改成功", {icon: 6}, function () {
                        // 获得frame索引
                        var index = parent.layer.getFrameIndex(window.name);
